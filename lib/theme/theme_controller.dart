...
///
/// 职责：
/// - 持有可动态修改的种子色（defaultSeed）和页面背景色（defaultBg）；
/// - 持有全局 App 标题（globalAppTitle）；
/// - 提供 saveTheme() 将当前配色持久化到本地存储；
/// - 提供 loadTheme() 从本地存储恢复配色。
///
/// 注册时机：AppInitService._registerGlobalService() → `Get.put(ThemeController())`
/// 访问方式：任意页面 `Get.find<ThemeController>()` 或 `Get.find()`（隐式类型）
class ThemeController extends GetxController {
  ...
  /// 全局 App 标题（响应式）。
  ///
  /// 应用级标题，供 MyRootApp 的 GetMaterialApp.router 的 title 参数使用。
  /// 使用 RxString.obs 包裹，使其成为响应式变量，Obx 包裹后自动跟随变化重建 App。
  ///
  /// 典型用途：未来支持用户自定义 App 名称或换肤时动态修改标题。
  final RxString globalAppTitle = 'Volnex 高考志愿助手'.obs;

  /// 主题种子色（响应式）。
  ///
  /// 种子色通过 ColorScheme.fromSeed(seedColor) 生成完整配色方案，
  /// 控制所有 Material 组件（AppBar、按钮、输入框等）的主色调。
  ///
  /// 初始值 AppColor.defaultSeed，用户可自定义后通过 saveTheme() 持久化。
  final Rx<Color> defaultSeed = const Color(0xff156B5B).obs;

  /// 全局页面背景色（响应式）。
  ///
  /// 应用于 ThemeData.scaffoldBackgroundColor，控制所有页面背景色。
  /// 初始值 AppColor.defaultBg，用户可自定义后通过 saveTheme() 持久化。
  final Rx<Color> defaultBg = const Color(0xffF6F8F7).obs;

  /// 将当前配色（种子色 + 背景色）保存到本地持久化存储。
  ///
  /// 调用时机：用户在设置页修改主题后，保存偏好。
  /// SharedPreferences 的键名："seed_color" 和 "bg_color"，
  /// 存储值为 Color.value（即 ARGB 整数）。
  ...
  /// 从本地持久化存储加载已保存的配色到响应式变量。
  ///
  /// 调用时机：AppInitService._registerGlobalService() 初始化阶段。
  /// 如果 SharedPreferences 中有已保存的颜色值，则覆盖当前的默认值；
  /// 如果没有（首次安装），则保持初始值不变。
  ///
