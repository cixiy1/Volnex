          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              TextFormField(
                ...
                onChanged: (String value) =>
                    _candidateRank = int.tryParse(value) ?? _candidateRank,
              ),
              const SizedBox(height: 20),
              // 保存按钮：会触发父页面刷新并关闭弹窗
              SizedBox(
...
class RecommendationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<University> rush = universities
        .where((University u) => u.cutoffRank < rank)
        .take(2)
        .toList();
    final List<University> steady = universities
        .where((University u) => (u.cutoffRank - rank).abs() < 9000)
        .take(3)
        .toList();
    final List<University> safe = universities
        .where((University u) => u.cutoffRank > rank)
        .take(2)
        .toList();
    ...
    return ListView(
      ...
      children: <Widget>[
        ...
        Text(
          '$province · 全省位次 $rank · 依据近三年录取位次与招生计划',
...
class _UniversityLibraryPageState extends State<UniversityLibraryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<University> filtered = universities
        .where(
          (University university) =>
    ...
    return ListView(
      ...
      children: <Widget>[
          ...
          children: <String>['海滨校园', '园林城市', '现代校园', '山地校园']
              .map(
                (String tag) => FilterChip(
                  ...
                  selected: _query == tag,
                  onSelected: (_) =>
                      setState(() => _query = _query == tag ? '' : tag),
                ),
              )
              .toList(),
        ),
        ...
        ...filtered.map(
          (University university) => UniversityCard(
            university: university,
            candidateRank: widget.candidateRank,
          ),
        ),
      ],
    );
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: <Color>[
                university.color,
                university.color.withValues(alpha: 0.55),
              ],
            ),
